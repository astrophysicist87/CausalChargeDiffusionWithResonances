...
namespace sfn
{
	...
	int n_resonances = -1;
	const double AT = 1.0, tauFINAL = 6.35185, TFO = 150.0/1000.0;
	const double chiQ = 0.385217;
	...
	//		double pT_loc, double pphi_loc, double pY_loc, double Temp );
		...
		bool skip_thermal )
	{
		...
		Del_pY_pts = Del_pY_pts_in;
		n_pY_pts = Del_pY_pts.size();
		k_pts = k_pts_in;
		n_k_pts = k_pts.size();
		double Del_pY_min = Del_pY_pts[0];
		double Del_pY_max = Del_pY_pts[n_pY_pts-1];
	...
	//if (1) exit (8);
		...
		//output script F_n vector for testing/checking...
		/*for (int ipY = 0; ipY < n_pY_pts; ipY++)
		for (int ik = 0; ik < n_k_pts; ik++)
		{
			//if (ipY != (n_pY_pts-1)/2)
			...
			//if (ik != (n_k_pts-1)/2)
			//	continue;
			complex<double> z = scriptFn_vector[scriptFn_vector_indexer(ik, (n_pY_pts-1)/2)]
	...
	void set_up_misc()
	{
		...
		//gauss_quadrature(n_xi_pts, 1, 0.0, 0.0, -1.0, 1.0, xi_pts, xi_wts);
		...
		gauss_quadrature(n_pT_pts, 5, 0.0, 0.0, 0.0, 5.0*n_pT_pts/15.0, pT_pts, pT_wts);
	//for (int ipT = 0; ipT < n_pT_pts; ++ipT)
	//	cout << "pT_pts[" << ipT << "] = " << pT_pts[ipT] << endl;
	...
	{
		double result = 0.0;
		//estimate half-width and center to get more accurate integration
		//double hw = (alpha < 1.0) ? 5.0/sqrt(alpha) : 7.5/sqrt(alpha);
		//double cen = 0.0;

		for (int ixi = 0; ixi < n_xi_pts; ixi++)
		{
			//double xi_pt = cen + hw * xi_pts[ixi];
			//result += hw * xi_wts[ixi] * cos(kpt * xi_pt) * f(xi_pt, alpha);
	...
	complex<double> thermal_FT_spectra(readindata::particle_info * particle, double kpt, double PT, double PPHI, double PY)
	{
		...
		//double prefactor = 1.0/(M_PI*chiQ);
		double prefactor = 1.0/(2.0*M_PI*TFO*TFO*TFO*chiQ);	//for checking purposes only!!!

	//cout << "CHECK: " << setw(20) << setprecision(16) << kpt << "   " << MT/TFO << "   " << get_integral(kpt, MT/TFO) << endl;
	//if (1) exit(8);
			...
			for (int ipY = 0; ipY < tmp_n_pY_pts; ipY++)
			{
				tmp_full_resonance_spectra_re[res_FIX_K_vector_indexer(iRes, ipT, ipphi, ipY)]
					= Cal_dN_dypTdpTdphi_toy_func(
						...
						&all_particles,
						pT_pts[ipT], pphi_pts[ipphi], tmp_Del_pY_pts[ipY], TFO );
			}
	...
	//time ./run_CCD 1 1 100 . 1 > test.out &
